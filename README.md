...
```bash
cmake -DCMAKE_BUILD_TYPE=Release -DPLATFORM=win -DARCH=x64 -B build
cmake --build build
```
...
`src/vbscript_test.c` contains a complete working example. The key steps are:
...
static void log_callback(libvbscript_log_level_t level, const char* format, va_list args)
{
   vprintf(format, args);
   printf("\n");
}

libvbscript_callbacks_t cb = { .log = log_callback };
libvbscript_init(&cb);
```

Including `libvbscript.h` defines `__LIBVBSCRIPT__`, which configures the Wine headers for library mode.
...
See `src/vbscript_test.c` for a complete minimal implementation.
...
Your `IDispatch` implementation maps method names to DISPIDs in `GetIDsOfNames` and handles calls in `Invoke`. See the `HostObj_*` functions in `src/vbscript_test.c`.
